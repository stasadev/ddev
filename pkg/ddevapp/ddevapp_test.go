...
\n\$settings['config_staging_class'] = 'ConfigDatabaseStorage';\n" >> /var/www/html/settings.php`,
		...
		{
			Name:                          "TestPkgLaravel",
			SourceURL:                     "https://github.com/ddev/test-laravel/archive/refs/tags/12.0.1.0.tar.gz",
			ArchiveInternalExtractionPath: "test-laravel-12.0.1.0/",
			FilesTarballURL:               "",
			FilesZipballURL:               "",
			DBTarURL:                      "https://github.com/ddev/test-laravel/releases/download/12.0.1.0/db.sql.tar.gz",
			DBZipURL:                      "",
			FullSiteTarballURL:            "",
	...
	for _, dbTypeVersion := range serverVersions {
		...
		for _, tool := range []string{"mysql", "mysqladmin", "mysqldump"} {
			...
			if dbType == nodeps.MariaDB {
				...
				// But for anything below 10.11 we install the 10.11 client.
